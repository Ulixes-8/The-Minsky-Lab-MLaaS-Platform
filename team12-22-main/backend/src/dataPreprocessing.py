# The code provided is a collection of functions which can help prepare a dataset for Neural Network Training. Here is the summary of the different functions in the code:
# 1- Is_classification function which returns True if the final column of a pandas DataFrame is categorical and False otherwise.
# 2- get_independent_variables function which splits a pandas DataFrame into features(X) and the target column/y.
# 3- get_dependent_variable function which gets the target column/y of a pandas DataFrame. 
# 4- classes function which returns a response string detailing the unique categories to be predicted if the target column of a pandas DataFrame is categorical.
# 5- remove_missing_values function which drops all rows with missing values in a pandas DataFrame.
# 6- remove_missing_values_verbose function which returns a string that explains the number of rows dropped due to missing values.
# 7- remove_infinite_values function which replaces infinite values in a pandas DataFrame with NaN, then drops all rows with NaN.
# 8- remove_infinite_values_verbose function which returns a string that explains the number of rows dropped due to infinite values.
# 9- remove_duplicates function which drops all duplicate rows in a pandas DataFrame.
# 10- remove_duplicates_verbose function which returns a string confirming that all duplicate rows have been removed along with the number of rows remaining.
# 11- num_categorical_variables function which returns the number of categorical variables in a pandas DataFrame.
# 12- encode_categorical_variables function which encodes categorical variables in a pandas DataFrame either using one-hot encoding if there are less than 15 categories, otherwise using label encoding.
# 13- encode_categorical_variables_verbose function which returns a string that explains whether any categorical data has been encoded or not.
# 14- normalize_data function which standardizes values in a pandas DataFrame using Standard Scaler.
# 15- normalize_data_verbose function which returns a string that explains the effect of the normalization, such as maximum, minimum, or average.
# 16- rank_features function which implements Random Forest Classifier or Regressor to rank features based on their importance and returns only the top features having an importance score of greater than 0.01.
# 17- rank_features_verbose which returns a string that lists the top-ranked features and their importance scores.
# 18- pre_process function which applies all of the above functions in the specified order to a pandas DataFrame and returns the pre-processed data in the form of features and target column/y.
...
def first_image_generator(df):
    ...
    alt_data = "there are {:.2f} missing values, {:.2f} duplicates and {:.2f} infinite values".format(float(number_of_missing_values), float(number_of_duplicates), float(number_of_infinity_values))


    return alt_data, encoded_image_data
...
def impute_missing_values_verbose(df):
    if (df.isnull().sum().sum() >= 1):
        response = "We've imputed all missing values with the mean of their respective columns."
    else:
        response = "Your dataset has no missing values."
    return response

def remove_missing_values_verbose(df):
    response = "We've dropped all rows with missing values. \n \n"
    return response
...
def remove_infinite_values_verbose(df):
    ...
    if (df.isin([np.inf, -np.inf]).sum().sum() >= 1):
        response = "We've dropped all rows with infinite values."
    else: 
        response = "Your dataset has no infinite values. \n \n"
    return response
...
def remove_duplicates_verbose(df):
    if (df.duplicated().sum() >= 1):
        response = "It seems that your dataset has duplicate rows. If this was a mistake during data entry, we recommend removing them and reuploading your dataset. \n \n"
    else:
        response = "Your dataset has no duplicate rows.\n \n"
    return response
...
def encode_categorical_variables_verbose(df):
    ...
    if num_categorical > 0:
        ...
        response = "Your dataset has no categorical variables and so encoding is not necessary. \n \n"
    return response
...
def pre_process(df):
    ...
    remove_missing_values_verbose(df)
    
    df = remove_infinite_values(df)
    remove_infinite_values_verbose(df)
    
    df = remove_duplicates(df)
    remove_duplicates_verbose(df)
    ...
    #Encode
    encode_categorical_variables_verbose(X)
    X = encode_categorical_variables(X)
    
    #Normalize
    X_normalized = normalize_data(X)
    normalize_data_verbose(X_normalized)
